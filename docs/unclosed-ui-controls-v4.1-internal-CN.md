...
| `profile.quick.lesson-account` | 个人中心课时账户 | 页面展示剩余课时/收费记录 | 与家长 `lesson-account`、学生 `lesson-account` 没有前端对账提示 | 增加“最近一次同步时间/来源” |
...
| `culture-wall.prepare-upload-photo` | 成果馆 | 上传入口已接日志链 | 缺少真实上传进度和失败恢复 | 接上传中台后补进度、重试、大小限制提示 |
